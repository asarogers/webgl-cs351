...
async function loadGLSLFiles() {
    g_vshader = await fetch('/live_demo_vertex.glsl').then(response => response.text()).then((x) => x)
    g_fshader = await fetch('/live_demo_fragment.glsl').then(response => response.text()).then((x) => x)
...
function startRendering() {
    ...
    g_modelMatrix = g_modelMatrix.setScale(.006, .006, -.006)
